    ...
    public function FtpUpload() {
      if (session_status() == PHP_SESSION_NONE) {
          session_start();
			...
			// $api_data = $this->CallAPI('get','https://app.influway.com/app/getloginUser');

			// echo json_encode([
			// 	'status' => 200,
			// 	'message' => 'Uploaded successfully',
			// 	'message' => 'https://app.influway.com/app/getloginUser',
            ...
            $dataPost = $_POST['data'];
            // my code
            // $json_data = json_decode($dataPost);
            $data = json_decode($dataPost);
			
            if(isset($_SESSION['user'])){
              // echo $data->pages[0]->title;
              $data->pages[0]->title= $_SESSION['user']['username'];
              // echo $data->pages[0]->title;
            }
      ...
      // echo json_encode($data);
        ...
        if (session_status() == PHP_SESSION_NONE) {
          session_start();
				...
				$output_dir = dirname(__FILE__) . $_POST['dir'].'/'.$userName;
				// $output_dir = dirname(__FILE__) . $_POST['dir'].$userName;
				// echo json_encode([
				// 	'status' => 200,
...
EOT;
      ...
      $con = new mysqli("localhost","root","mypass","boss_influway");
      // $con = mysqli_connect("localhost","root","","jango_e_commerce");
      // Check connection
      ...
      // Perform query
      // $sql = "UPDATE users SET lastname='$data' WHERE id=3";
      // $result = $mysqli -> query("UPDATE users SET user_own_site='$data' WHERE id=3");
      // $data = json_decode($dataPost);
      ...
      $result = mysqli_query($con,"UPDATE users SET user_own_site=$json_data WHERE id=$user_id");
      // $result = mysqli_query($con,"UPDATE users SET user_own_site='asdasd' WHERE id=3");
      if ($result) {
        // echo "Record updated successfully";
        $_SESSION['user']['user_own_site'] = $data;
        echo $data;
        // Free result set
        // $result -> free_result();
      }
      else {
        echo "Error updating record: " . mysqli_error($result);
      ...
      // original code
			...
			// $filename = "tmp/" . $_SESSION['user']['username'] . "_project.zip";
			// // $filename = "tmp/" . 'hafiz' . "_project.zip";
			// $zip = new ZipArchive();
      ...
      // echo header('http://localhost/influwaymaker/ajax.php?mode=ftpUpload');
      // echo header("Location: http://localhost/influwaymaker/ajax.php?mode=ftpUpload"); 
      // echo 'ftpupload start';
      // $this->FtpUpload();
      // echo 'ftpupload end';
			// exit();
      ...
      $result = mysqli_query($con,"SELECT user_own_site FROM users WHERE id=$user_id");
      // $result = mysqli_query($con,"SELECT user_own_site FROM users WHERE id=3");
      if ($result) {
        // echo $user_id;
        // echo "Record updated successfully";
        ...
        // echo json_encode($row['user_own_site']);
        echo $row['user_own_site'];

        // Free result set
        // $result -> free_result();
      }
      else {
        echo "Error reading record: " . mysqli_error($result);
        // echo "Error updating record: ";
      }
      // $this->_upload_file($this->_base_path .'/tmp/', array( '.zip', '.supra' ), 'import');
      exit();
...
//								var_dump('video/gallery/' . $video . $type);die;
		...
		public function Download() {
            ...
            echo $file_name = $this->saveSiteToTmp($dataPost);
